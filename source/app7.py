...
def train_epoch(model, loader, optimizer, device, scaler=None):
    ...
    for inputs, targets in loader:
        ...
        loss = criterion_mse(outputs, targets) * criterion_mse_lambda #LAMBDA_DECOUPLE * (decouple_loss / (INPUT_LENGTH + PRED_LENGTH)) 
        # DA PROVARE IN SEGUITO
        #loss += criterion_fl_from_normalized(logits, targets, 0.5, 0.5)
...
def save_predictions_gt(predictions, output_dir="outputs"):
