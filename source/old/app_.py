...
def calculate_metrics(preds, targets, threshold_dbz=15):
    preds = preds.cpu().numpy().squeeze()  # Da torch tensor a NumPy
    ...
    # SSIM: Iterare su batch e frames temporali
    ssim_values = []
    for b in range(preds_dbz.shape[0]):  # Batch loop
        for t in range(preds_dbz.shape[1]):  # Time step loop
            try:
                ssim_t = ssim(
                    preds_dbz[b, t], targets_dbz[b, t],  # Prende (256, 256)
                    data_range=70.0, win_size=3, multichannel=False
                )
                ssim_values.append(ssim_t)
            except ValueError:
                print(f"Errore nel calcolo SSIM per batch {b}, frame {t}.")
    ...
    # Gestione casi speciali per la matrice di confusione
    if cm.shape == (1, 1):
...
# def calculate_metrics(preds, targets, threshold_dbz=15):
#     print("Dimensioni preds (prima della squeeze):", preds.shape)
#     print("Dimensioni targets (prima della squeeze):", targets.shape)
#     preds = preds.cpu().numpy().squeeze()
#     targets = targets.cpu().numpy().squeeze()
#     print("Dimensioni preds (dopo la squeeze):", preds.shape)
#     print("Dimensioni targets (dopo la squeeze):", targets.shape)

#     # Denormalizza i valori (da 0-1 a 0-70 dBZ)
#     targets_dbz = targets * 70.0
#     preds_dbz = preds * 70.0
#     print("Dimensioni targets_dbz (denormalizzati):", targets_dbz.shape)
#     print("Dimensioni preds_dbz (denormalizzati):", preds_dbz.shape)

#     # Calcola MAE
#     mae = np.mean(np.abs(preds_dbz - targets_dbz))

#     # Verifica dimensioni per SSIM
#     min_dim = min(preds_dbz.shape[-2], preds_dbz.shape[-1])  # Altezza e larghezza minima

#     print("Dimensione minima (min_dim):", min_dim)
...
#             )
#         except ValueError:
#             ssim_val = 0.0  # Evita NaN
#     else:
#         ssim_val = 0.0  # Se troppo piccolo, assegna il minimo SSIM

#     # Binarizza con soglia di 15 dBZ
#     preds_bin = (preds_dbz > threshold_dbz).astype(np.uint8)
#     targets_bin = (targets_dbz > threshold_dbz).astype(np.uint8)
#     print("Dimensioni preds_bin (binarizzate):", preds_bin.shape)
#     print("Dimensioni targets_bin (binarizzate):", targets_bin.shape)

#     # Calcola la matrice di confusione
#     cm = confusion_matrix(targets_bin.flatten(), preds_bin.flatten())
#     print("Matrice di confusione (forma):", cm.shape)
#     print("Matrice di confusione (valori):\n", cm)
