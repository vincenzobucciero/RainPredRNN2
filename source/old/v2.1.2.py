...
class SpatiotemporalLSTMCell(nn.Module):
    ...
    def forward(self, x, h_c, c_c, m_c):
        ...
        decouple_loss = torch.mean(torch.abs(delta_c_conv - delta_m_conv)) / (self.num_hidden * x.size(0))
...
class PredRNN_Block(nn.Module):
    ...
    def forward(self, inputs, hidden_states):
        ...
        for l in range(self.layers):
            ...
            m_prev = hidden_states[2][l-1] if l > 0 else None  # M dal layer precedente (l-1)
        ...
        for l in reversed(range(self.layers-1)):
            m_lower = new_memory[l+1]  # M dal layer inferiore (l+1)
    ...
    def forward(self, input_sequence, pred_length, teacher_forcing=False):
        ...
        for t in range(seq_len + pred_length):
            if t < seq_len:
                ...
                # Usa teacher forcing o previsione precedente
                if teacher_forcing and self.training:
                    x, skip = encoder_features[t] if t < seq_len else (None, None)
                else:
                    prev_pred = predictions[-1] if predictions else input_sequence[:, -1]
                    enc_out, _ = self.encoder(prev_pred.unsqueeze(1))
                    x = enc_out.squeeze(1)
                    skip = encoder_features[-1][1] if encoder_features else None
