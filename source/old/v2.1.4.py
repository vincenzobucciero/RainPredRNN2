...
class PredRNN_Block(nn.Module):
    ...
    def forward(self, input_sequence, h_t, c_t, m_t):
        ...
        for t in range(seq_len):
            # Propagazione verticale (bottom-up) per ogni timestep
            for l in reversed(range(len(self.cells))):
                # Per il primo livello, usa l'input del timestep corrente
                if l == 0:
                    input_current = input_sequence[:, t]
                else:
                    input_current = h_t[l-1]  # Output del livello precedente
                
                # Aggiorna h, c, m per il livello l
                h_t[l], c_t[l], m_t[l] = self.cells[l](
                    input_current,
                    h_t[l],
                    c_t[l],
                    m_t[l],
                    m_t[l+1] if l < len(self.cells)-1 else None  # Memoria del livello superiore
                )
            
            # Propagazione orizzontale (timestep successivo)
            output_inner.append(h_t[-1])  # Output dell'ultimo livello
...
class RainPredRNN(nn.Module):
    ...
    def forward(self, input_sequence, pred_length, teacher_forcing=False):
        ...
        # Inizializza gli stati persistenti per ogni batch
