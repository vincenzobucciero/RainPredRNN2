...
class SpatiotemporalLSTMCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, filter_size):
        ...
        # Convoluzioni per i gate combinati
        ...
        self.conv_m = nn.Conv2d(hidden_dim, hidden_dim * 3, 
                               kernel_size=1)
        # Convoluzioni per la decoupling loss
        self.conv_c_decouple = nn.Conv2d(hidden_dim, hidden_dim, 
                                       kernel_size=1)
        self.conv_m_decouple = nn.Conv2d(hidden_dim, hidden_dim, 
                                       kernel_size=1)

    def forward(self, x, h_prev, c_prev, m_prev, m_upper=None):
        # Aggiorna memoria spaziale con flusso bottom-up
...
class PredRNN_Block(nn.Module):
    ...
    def forward(self, input_sequence, h_t, c_t, m_t):
        ...
        for t in range(seq_len):
            ...
            for l in reversed(range(len(self.cells))):
                # Per il primo layer, usa l'input del timestep corrente
                if l == 0:
                    input_current = input_sequence[:, t]
                else:
                    input_current = h_t[l-1]  # Output del layer precedente
                # Aggiorna stati per il layer l
                h_t[l], c_t[l], m_t[l], _ = self.cells[l](
                    input_current,
                    h_t[l],
                    c_t[l],
                    m_t[l],
                    m_t[l+1] if l < len(self.cells)-1 else None  # Memoria del layer superiore
                )
            
            # Fase 2: Propagazione top-down (da layer basso ad alto)
            for l in range(1, len(self.cells)):
                # Usa l'output del layer precedente (l-1) per aggiornare il layer l
                ...
                h_t[l], c_t[l], m_t[l], _ = self.cells[l](
                    ...
                    c_t[l],
                    m_t[l],
                    m_t[l-1] if l > 0 else None  # Memoria del layer inferiore
                )
            
            output_inner.append(h_t[-1])  # Output dell'ultimo layer
        return torch.stack(output_inner, dim=1), h_t, c_t, m_t
...
class RainPredRNN(nn.Module):
    ...
    def forward(self, input_sequence, pred_length, teacher_forcing=False):
        ...
        # Codifica gli input e salva le skip connections
        encoder_skips = []
        for t in range(seq_len):
        ...
        for t in range(seq_len + pred_length):
            if t < seq_len:
                ...
                # Fase di predizione: usa la precedente predizione
                if teacher_forcing and self.training:
                    # Usa l'input reale (teacher forcing)
                    x = enc_out  # Ultimo enc_out disponibile
                else:
                    # Codifica la precedente predizione per ottenere le nuove features
                    prev_pred = predictions[-1] if predictions else input_sequence[:, -1]
                    enc_out, skip = self.encoder(prev_pred)
                    encoder_skips.append(skip)  # Aggiorna le skip connections
                    x = enc_out
            ...
            if t >= seq_len:
                # Decodifica l'output dell'RNN usando la skip connection corrente
