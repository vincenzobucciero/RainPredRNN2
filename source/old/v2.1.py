...
class PredRNN_Block(nn.Module):
    ...
    def forward(self, inputs, hidden_states):
        ...
        for l in range(self.layers):
            ...
            # Per i layer > 0, prendi M dal layer precedente
            m_prev = new_memory[l-1] if l > 0 else None
            # Aggiorna gli stati
            ...
            inputs = h_new  # Input per il layer successivo
        
        # ===== FASE BOTTOM-UP (layer N-1 → layer 0) =====
        for l in reversed(range(self.layers - 1)):
            # Prendi la memoria dal layer inferiore (l+1)
            m_lower = new_memory[l + 1]
            # Concatena e applica la fusione convoluzionale
            fused = torch.cat([new_memory[l], m_lower], dim=1)
            new_memory[l] = self.conv_fuse(fused)
    ...
    def forward(self, input_sequence, pred_length):
        ...
        for t in range(seq_len + pred_length):
            if t < seq_len:
                ...
                # Usa la skip dell'ultimo input reale
                _, skip = encoder_features[-1]
                # Codifica la predizione precedente (solo feature map)
                prev_pred = predictions[-1] if predictions else torch.zeros_like(targets)
                enc_out, _ = self.encoder(prev_pred)  # Ignora la nuova skip
                x = enc_out
