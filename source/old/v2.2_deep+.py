...
class UNet_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = self.enc3(x_pooled2)
        x_pooled3 = self.pool(x3)
        
        x4 = self.enc4(x_pooled3)
        return x4, x3, x2, x1
...
class RainPredRNN(nn.Module):
    ...
    def forward(self, input_sequence, pred_length):
        ...
        for l in range(self.num_layers):
            h_t.append(torch.zeros(batch_size, self.num_hidden, h//16, w//16).to(input_sequence.device))
            c_t.append(torch.zeros(batch_size, self.num_hidden, h//16, w//16).to(input_sequence.device))
            m_t.append(torch.zeros(batch_size, self.num_hidden, h//16, w//16).to(input_sequence.device))
        
        encoder_features = []
        for t in range(seq_len):
            enc_out, skip3, skip2, skip1 = self.encoder(input_sequence[:, t])
            encoder_features.append((enc_out, skip3, skip2, skip1))
        
        predictions = []
        total_decouple_loss = 0
        for t in range(seq_len + pred_length):
            if t < seq_len:
                x, skip3, skip2, skip1 = encoder_features[t]
            else:
            ...
            #     skip1 = skip1 if predictions else torch.zeros_like(skip1)  
            #     skip2 = skip2 if predictions else torch.zeros_like(skip2)  
            #     skip3 = skip3 if predictions else torch.zeros_like(skip3)  
            
            rnn_out, (h_t, c_t, m_t), decouple_loss = self.rnn_block(x, [h_t, c_t, m_t])
            total_decouple_loss += decouple_loss
            
            if t >= seq_len:
                pred = self.decoder(rnn_out, [skip3, skip2, skip1])
