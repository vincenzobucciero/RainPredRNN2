...
class RainPredRNN(nn.Module):
    ...
    def forward(self, input_sequence, pred_length, teacher_forcing=False):
        ...
        return torch.stack(predictions, dim=1), total_decouple_loss.unsqueeze(0)
...
'''def train_epoch(model, loader, optimizer, criterion, device):
    ...
    for inputs, targets in loader:
        ...
        with autocast():
            ...
            # Assicurati che decouple_loss sia un tensore scalare
            if isinstance(decouple_loss, list):  
                decouple_loss = torch.stack(decouple_loss).mean()
            elif not isinstance(decouple_loss, torch.Tensor):  
                decouple_loss = torch.tensor(decouple_loss, device=device)

            loss = criterion(outputs, targets) + LAMBDA_DECOUPLE * (decouple_loss.mean() / (INPUT_LENGTH + PRED_LENGTH))
    ...
    for inputs, targets in loader:
        ...
        with autocast():
            ...
            if isinstance(decouple_loss, torch.Tensor):
                decouple_loss = decouple_loss.mean()  # Aggrega su tutte le GPU
            else:
                decouple_loss = torch.tensor(decouple_loss, device=device)  # Converti in tensore
