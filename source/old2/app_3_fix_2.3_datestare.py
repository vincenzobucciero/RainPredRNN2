...
def train_epoch(model, loader, optimizer, device, scaler=None):
    ...
    for inputs, targets in loader:
        ...
        with torch.cuda.amp.autocast(enabled=(scaler is not None)):
            ...
            # Verifica se SSIM loss è un tensore o un valore scalare
            ssim_loss = 0.0
            ...
            # Se SSIM è un tensore, prendiamo la media per renderlo scalare
            ssim_loss = ssim_loss / outputs.shape[1] if isinstance(ssim_loss, torch.Tensor) else torch.tensor(ssim_loss, device=device)
            ...
            loss = criterion_mae(outputs, targets).mean() + 0.5 * ssim_loss + LAMBDA_DECOUPLE * (decouple_loss / (INPUT_LENGTH + PRED_LENGTH))
